...
while IFS= read -r instanceid; do
	...
	aws cloudwatch put-metric-alarm --alarm-name disk-used-$instanceid --alarm-description "Alarm when $instanceid disk used percentage is above 85%" --metric-name disk_used_percent --namespace runotp-prod-cwagent --statistic Average --period 60 --threshold 70 --comparison-operator GreaterThanThreshold --dimensions "Name=InstanceId,Value=$instanceid" --evaluation-periods 2 --alarm-actions $action --unit Percent --tags Key=Instance,Value=$instanceid Key=Description,Value="SOC2 Compliance" --profile $MFA_PROFILE
done < $file
